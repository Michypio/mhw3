...
document.addEventListener('DOMContentLoaded', () => {
    ...
    giocatori
        ...
        .forEach(giocatore => {
            ...
            ruolo.textContent = giocatore.ruolo.substring(0, 3).toUpperCase(); 
    ...
    const headerInfoAll = document.createElement('div'); 
...
function updateOddsUI(partitaElement, match) {
    ...
    const quoteContainer = template.content.cloneNode(true);
    
    if (match && match.bookmakers && match.bookmakers[0]) {
        const outcomes = match.bookmakers[0].markets[0].outcomes;
        quoteContainer.querySelector('.quota-1').textContent = outcomes[0].price.toFixed(2);
        quoteContainer.querySelector('.quota-x').textContent = outcomes[1].price.toFixed(2);
        quoteContainer.querySelector('.quota-2').textContent = outcomes[2].price.toFixed(2);
        
        if (match.bookmakers[0].title) {
            quoteContainer.querySelector('.nome-bookmaker').textContent = match.bookmakers[0].title;
        }
    } else {
        const fallbackOdds = {
            homeWin: 1.85 + Math.random() * 0.5,
        ...
        quoteContainer.querySelector('.quota-1').textContent = fallbackOdds.homeWin.toFixed(2);
        quoteContainer.querySelector('.quota-x').textContent = fallbackOdds.draw.toFixed(2);
        quoteContainer.querySelector('.quota-2').textContent = fallbackOdds.awayWin.toFixed(2);
        
    
        const bookmakerEl = quoteContainer.querySelector('.bookmaker');
        if (bookmakerEl) bookmakerEl.remove();
    }
    
    partitaElement.appendChild(quoteContainer);
}
